...
public class SuspectHandler
{
    ...
    public static void GetLocation(string CarType, string LicencePlate, Vehicle Car, Ped Witness)
    {
        ...
        GameFiber.StartNew(delegate
        {
            ...
            GameHandler.DispatchMessage(SuspectHandlerTranslation.TEXT[12]);//"Be Advised! Vehicle returned as: ~r~ STOLEN");
            GameFiber.Wait(2000);
            Game.LocalPlayer.Character.Tasks.PlayAnimation("random@arrests", "generic_radio_chatter", 1f, AnimationFlags.UpperBodyOnly | AnimationFlags.SecondaryTask);
            GameFiber.Wait(2000);
    ...
    public static string RandomLicensePlate(Vehicle vehicle)
    {
        // 2 Zufällige Zahlen (10-99)
        string prefix = MathHelper.GetRandomInteger(10, 100).ToString();

        // 3 Zufällige Buchstaben (ABC)
        const string chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        char[] letters = new char[3];
        for (int i = 0; i < 3; i++)
        {
            letters[i] = chars[MathHelper.GetRandomInteger(chars.Length)];
        }

        // 3 Zufällige Zahlen (100-999)
        string suffix = MathHelper.GetRandomInteger(100, 1000).ToString();

        // Alles zusammenfügen
        string finalPlate = prefix + new string(letters) + suffix;

        if (vehicle.Exists())
        {
            vehicle.LicensePlate = finalPlate;
        }
