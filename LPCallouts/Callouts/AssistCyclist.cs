...
{
    [CalloutInfo("MVA_Cyclist.Spawn.Low", CalloutProbability.Low)]
    class AssistCyclist_LOW : AssistCyclist
    {

    }
    [CalloutInfo("MVA_Cyclist.Spawn.Medium", CalloutProbability.Medium)]
    class AssistCyclist_MED : AssistCyclist
    {

    }
    [CalloutInfo("MVA_Cyclist.Spawn.High", CalloutProbability.High)]
    class AssistCyclist_HIGH : AssistCyclist
    {

    }

    class AssistCyclist : Callout, ISuspect
    {
        ...
        public Globals.SuspectDialog _suspect_dialog;
        ...
        public override bool OnBeforeCalloutDisplayed()
        {
            try
            {
                ...
                List<Globals.Scenery> CalloutsInRange = Content.CalloutLocations.Where(t => t._type == Globals.CalloutType.CyclistScene && t._poi.DistanceTo(CurrentPlayerPosition) <= GameHandler.ini_radius).ToList();
            ...
            GameFiber.StartNew(delegate
            {
                ...
                switch (endingvariant)
                {
                    case Globals.CalloutEnd.NORMAL:
                        ...
                        try
                        {
                            SuspectHandler.GenerateSuspect(this, ref _suspect_veh, ref _suspect_ped, ref _suspect_location, ref _suspect_dialog, _area._region, Globals.CalloutType._Default);
                        }
                        catch
                        {
            ...
            if (_contentisloaded == true)
            {
                ...
                switch (statusmachine)
                {
                    ...
                    case Globals.PlayerState.ONROUTE:
                        ...
                        _ped_civ1 = Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 3)._model;
                        _ped_civ2 = Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 4)._model;
                        _veh_civ1 = Content.CarList.First(t => t._calloutid == _area._calloutid && t._id == 4)._model;
                        _veh_civ2 = Content.CarList.First(t => t._calloutid == _area._calloutid && t._id == 5)._model;

                        if (Game.LocalPlayer.Character.DistanceTo(_ve3_poi) < 50f)
                        {
                            HudHelpers.DisplaySubtitle(AssistCyclistTranslation.TEXT[0].Replace("{0}", $"~o~'{GameHandler.ini_action.ToString()}'~w~"), GameHandler.ini_displaytime);//"Talk to the officer and witnesses by pressing ~o~'" + GameHandler.ini_action.ToString() + "'~w~ to gain information about the accident.", GameHandler.ini_displaytime);
                    ...
                    case Globals.PlayerState.TO_SUSPECT:
                        ...
                        {
                            SuspectHandler.Conversation(_ped_suspect, _blip_suspect, ref _pursuit, _suspect_dialog, _pursuitcreated, ref _mlog_loop4, ref _talking_suspect);
                        }
                        ...
                        if (Game.LocalPlayer.Character.LastVehicle.Exists() && _veh_suspect.Exists())
                        {
                            ...
                            {
                                HudHelpers.DisplaySubtitle(AssistCyclistTranslation.TEXT[1]/*"Suspect had heard your sirens and is fleeing their home."*/, GameHandler.ini_displaytime);
                ...
                {
                    if (_talking_cop != true || _talking_witness1 != true || _talking_witness2 != true)
                    {
            ...
            if (Rage.Game.IsKeyDown(GameHandler.ini_action))
            {
                ...
                switch (_mlog_loop2)
                {
                    ...
                    case 6:
                        ...
                        FiberHandler.BackToAnimation(_ped_civ1, Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 3)._heading, "missmic_4premierejimwaitbef_prem", "wait_idle_b", AnimationFlags.Loop);
                        break;
                }
            }
        }
            ...
            if (Rage.Game.IsKeyDown(GameHandler.ini_action))
            {
                ...
                switch (_mlog_loop3)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        GameHandler.PlayerChat(3, AssistCyclistTranslation.TEXT[2]/*A*/ + " ~y~" + _veh_suspect.Model.Name + Dialog, GameHandler.ini_displaytime);
                        _mlog_loop3 = 4;
                        break;
                    case 4:
                        GameHandler.PlayerChat(1, AssistCyclistTranslation.TEXT[3].Replace("{0}", "~y~" + _veh_suspect.Model.Name + "~w~")/*"Did you see the "~y~" + _veh_suspect.Model.Name + "~w~'s license plate?"*/, GameHandler.ini_displaytime);
                        _mlog_loop3 = 5;
                        break;
                    case 5:
                        GameHandler.PlayerChat(3, AssistCyclistTranslation.TEXT[4].Replace("{0}", "~y~" + _veh_suspect.LicensePlate)/*"Yes I did. It was ~y~"*/, GameHandler.ini_displaytime);
                        _mlog_loop3 = 6;
                        switch (endingvariant)
                        {
                            case Globals.CalloutEnd.NORMAL:
                    ...
                    case 8:
                        ...
                        FiberHandler.BackToAnimation(_ped_civ2, Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 4)._heading, "missfbi3_party_d", "stand_talk_loop_b_male1", AnimationFlags.Loop);
                        switch (endingvariant)
                        {
                            case Globals.CalloutEnd.NORMAL:
