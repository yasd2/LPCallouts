...
{
    [CalloutInfo("MVA_CornerCut.Spawn.Low", CalloutProbability.Low)]
    class AssistCornerCut_LOW : AssistCornerCut
    {

    }
    [CalloutInfo("MVA_CornerCut.Spawn.Medium", CalloutProbability.Medium)]
    class AssistCornerCut_MED : AssistCornerCut
    {

    }
    [CalloutInfo("MVA_CornerCut.Spawn.High", CalloutProbability.High)]
    class AssistCornerCut_HIGH : AssistCornerCut
    {

    }

    class AssistCornerCut : Callout, ISuspect
    {
        ...
        public bool _talking_victim = false;
        ...
        public override bool OnBeforeCalloutDisplayed()
        {
            try
            {
                ...
                List<Globals.Scenery> CalloutsInRange = Content.CalloutLocations.Where(t => t._type == Globals.CalloutType.CornerCut && t._poi.DistanceTo(CurrentPlayerPosition) <= GameHandler.ini_radius).ToList();
            ...
            else
            {
                ...
                CalloutMessage = AssistCornerCutTranslation.CALLOUTMESSAGE;//"Assistance required";
            ...
            if (_contentisloaded == true)
            {
                ...
                switch (statusmachine)
                {
                    ...
                    case Globals.PlayerState.ONROUTE:
                        ...
                        _ped_paramedic1 = Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 7)._model;
                        _ped_civ1 = Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 4)._model;
                        _ped_victim = Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 3)._model;
                        _veh_victim = Content.CarList.First(t => t._calloutid == _area._calloutid && t._id == 4)._model;
                        _veh_civ1 = Content.CarList.First(t => t._calloutid == _area._calloutid && t._id == 5)._model;
                    ...
                    case Globals.PlayerState.TALK_COP:
                        ...
                        break;
                    //VALUE = 2 -> ARRIVED AT SCENE -> CONVERSATION VICTIM
                    case Globals.PlayerState.TALK_VICTIM:
                        if (Game.LocalPlayer.Character.DistanceTo(_ped_paramedic1.Position) < 2f)
                        {
                            Conversation_Victim();
                        }
                        break;
                    //VALUE = 3 -> ARRIVED AT SCENE -> CONVERSATION VICTIM
                ...
                //VICTIM
                if (_talking_victim == true)
                {
                    if (_player != Game.LocalPlayer.Character.Position)
                    {
                        float _faceangle = GameHandler.PedtoPlayerAngle(_ped_paramedic1);
                        Content.HumanList.Where(t => t._calloutid == _area._calloutid && t._id == 7).First()._model.Tasks.AchieveHeading(_faceangle);
                        _player = Game.LocalPlayer.Character.Position;
                    }
                }
                //WITNESS
        ...
        public void Conversation_Cop()
        {
            if (Rage.Game.IsKeyDown(GameHandler.ini_action))
            {
                ...
                switch (_mlog_loop1)
                {
                    ...
                    case 3:
                        GameHandler.PlayerChat(2, Dialog, GameHandler.ini_displaytime);
                        _blip_witness01 = _ped_paramedic1.AttachBlip();
                    ...
                    case 4:
                        ...
                        statusmachine = Globals.PlayerState.TALK_VICTIM;
                        GameHandler.PlayerChat(2, Dialog, GameHandler.ini_displaytime);
                        _ped_cop.Tasks.AchieveHeading(Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 1)._heading);
                        GameFiber.StartNew(delegate
                        {
                            GameFiber.Wait(2000);
                            _ped_cop.Tasks.PlayAnimation("amb@world_human_cop_idles@male@idle_b", "idle_d", 1f, AnimationFlags.Loop);
        ...
        public void Conversation_Victim()
        {
            if (Rage.Game.IsKeyDown(GameHandler.ini_action))
            {
                ...
                {
                    case 0:
                        _talking_victim = true;
                        GameHandler.PlayerChat(1, Dialog, GameHandler.ini_displaytime);
                        _mlog_loop2 = 1;
                        break;
                    case 1:
                        GameHandler.PlayerChat(4, Dialog, GameHandler.ini_displaytime);
                        _mlog_loop2 = 2;
                        break;
                    case 2:
                        GameHandler.PlayerChat(6, Dialog, GameHandler.ini_displaytime);
                        _mlog_loop2 = 3;
                        break;
                    case 3:
                        GameHandler.PlayerChat(1, Dialog, GameHandler.ini_displaytime);
                    ...
                    case 5:
                        GameHandler.PlayerChat(6, Dialog, GameHandler.ini_displaytime);
                        _blip_witness02 = _ped_civ1.AttachBlip();
                        _blip_witness02.Color = System.Drawing.Color.Orange;
                        _blip_list.Add(_blip_witness02);
                        _mlog_loop2 = 6;
                        break;
                    case 6:
                        _talking_victim = false;
                        GameHandler.RemoveBlip(_blip_witness01, _blip_list);
                        statusmachine = Globals.PlayerState.TALK_WITNESS1;
                        GameHandler.PlayerChat(6, Dialog, GameHandler.ini_displaytime);
                        _ped_paramedic1.Tasks.AchieveHeading(Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 7)._heading);
                        GameFiber.StartNew(delegate
                        {
                            GameFiber.Wait(2000);
                            _ped_paramedic1.Tasks.PlayAnimation("amb@medic@standing@kneel@idle_a", "idle_b", 1f, AnimationFlags.Loop);
                        }, FiberHandler._fiber_anim);
                        break;
                }
            }
            ...
            if (Rage.Game.IsKeyDown(GameHandler.ini_action))
            {
                ...
                switch (_mlog_loop3)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        GameHandler.PlayerChat(3, AssistCornerCutTranslation.TEXT[2] +/*"I think it was a*/ "~y~ " + _veh_suspect.Model.Name + "~w~" + Dialog, GameHandler.ini_displaytime);
                        _mlog_loop3 = 4;
                        break;
                    case 4:
                        GameHandler.PlayerChat(1, Dialog + " ~y~" + _veh_suspect.Model.Name, GameHandler.ini_displaytime);
                        _mlog_loop3 = 5;
                        break;
                    case 5:
                        GameHandler.PlayerChat(3, Dialog + " ~y~" + _veh_suspect.LicensePlate, GameHandler.ini_displaytime);
                        _mlog_loop3 = 6;
                        switch (endingvariant)
                        {
                            case Globals.CalloutEnd.NORMAL:
                                SuspectHandler.GetLocation(_veh_suspect.Model.Name, _veh_suspect.LicensePlate, _veh_suspect, _ped_suspect, _ped_civ1, _blip_suspect, _suspect_location._ped_position, SuspectData.FullName);
                                break;
                            case Globals.CalloutEnd.ABANDON:
                            case Globals.CalloutEnd.TRAFFICSTOP:
                                SuspectHandler.GetLocation(_veh_suspect.Model.Name, _veh_suspect.LicensePlate, _veh_suspect, _ped_civ1);
                                break;
                        }
                        break;
                    case 6:
                    ...
                    case 7:
                        ...
                        Globals.Humans _endanim = Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 4);
                        FiberHandler.BackToAnimation(_ped_civ1, _endanim._heading, _endanim._animdir, _endanim._animname, _endanim._animflag);
                        switch (endingvariant)
                        {
                            case Globals.CalloutEnd.NORMAL:
