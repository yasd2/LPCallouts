...
{
    [CalloutInfo("MVA_Drifter.Spawn.Low", CalloutProbability.Low)]
    class AssistAvoidDrifer_LOW : AssistAvoidDrifter
    {
    }

    [CalloutInfo("MVA_Drifter.Spawn.Medium", CalloutProbability.Medium)]
    class AssistAvoidDrifer_MED : AssistAvoidDrifter
    {
    }

    [CalloutInfo("MVA_Drifter.Spawn.High", CalloutProbability.High)]
    class AssistAvoidDrifer_HIGH : AssistAvoidDrifter
    {
    }

    class AssistAvoidDrifter : Callout, ISuspect
    {
        ...
        public override bool OnBeforeCalloutDisplayed()
        {
            try
            {
                ...
                List<Globals.Scenery> CalloutsInRange = Content.CalloutLocations.Where(t => t._type == Globals.CalloutType.DrifterScene && t._poi.DistanceTo(CurrentPlayerPosition) <= GameHandler.ini_radius).ToList();
            ...
            else
            {
                ...
                CalloutMessage = AssistAvoidDrifterTranslation.CALLOUTMESSAGE;//"Assistance required";
                ...
                switch (endingvariant)
                {
                    ...
                    case Globals.CalloutEnd.ABANDON:
                        ...
                        try
                        {
                            ...
                            _end_dialog_id = _idList[Main.LPCRandom.Next(_idList.Count())];
                        }
                        catch
                        {
                            ErrorHandler.LogMessage(this.GetType().Name + " Suspect vehicle could not be placed", 005);
        ...
        public override void Process()
        {
            if (_contentisloaded == true)
            {
                ...
                switch (statusmachine)
                {
                    ...
                    case Globals.PlayerState.CALLED:
                        ...
                        if (Game.LocalPlayer.Character.DistanceTo(_ve3_poi) < 250f)
                        {
                            ...
                            {
                                ErrorHandler.LogMessage(this.GetType().Name + " Scenery Models could not be loaded", 010);
                            }
                    ...
                    case Globals.PlayerState.ONROUTE:
                        ...
                        _veh_civ = Content.CarList.First(t => t._calloutid == _area._calloutid && t._id == 3)._model;

                        if (Game.LocalPlayer.Character.DistanceTo(_ve3_poi) < 50f)
                        {
                            HudHelpers.DisplaySubtitle(AssistAvoidDrifterTranslation.TEXT[0].Replace("{0}", $"~o~'{GameHandler.ini_action.ToString()}'~w~"/*"Talk to the officer and witnesses by pressing ~o~'" + GameHandler.ini_action.ToString() + "'~w~ to gain information about the accident."*/), GameHandler.ini_displaytime);
                    ...
                    case Globals.PlayerState.TO_SUSPECT:
                        ...
                        if (Game.LocalPlayer.Character.LastVehicle.Exists() && _veh_suspect.Exists())
                        {
                            ...
                            {
                                HudHelpers.DisplaySubtitle(AssistAvoidDrifterTranslation.TEXT[1]/*"Suspect had heard your sirens and is fleeing their home."*/, GameHandler.ini_displaytime);
                ...
                {
                    if (_talking_cop != true || _talking_victim != true)
                    {
            ...
            if (Rage.Game.IsKeyDown(GameHandler.ini_action))
            {
                ...
                switch (_mlog_loop1)
                {
                    ...
                    case 5:
                        ...
                        FiberHandler.BackToAnimation(_ped_cop, Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 1)._heading, "amb@world_human_cop_idles@male@idle_a", "idle_a", AnimationFlags.Loop);
                        break;
                }
            }
        }
            ...
            if (Rage.Game.IsKeyDown(GameHandler.ini_action))
            {
                ...
                switch (_mlog_loop3)
                {
                    ...
                    case 2:
                        ...
                        break;
                    case 3:
                        GameHandler.PlayerChat(4, AssistAvoidDrifterTranslation.TEXT[2]/*"A"*/ + " ~y~" + _veh_suspect.Model.Name + Dialog, GameHandler.ini_displaytime);
                        _mlog_loop3 = 4;
                        break;
                    case 4:
                        GameHandler.PlayerChat(1, Dialog + _veh_suspect.Model.Name + "~w~"+ AssistAvoidDrifterTranslation.TEXT[3]/*"'s license plate?"*/, GameHandler.ini_displaytime);
                        _mlog_loop3 = 5;
                        break;
                    case 5:
                        GameHandler.PlayerChat(4, Dialog + _veh_suspect.LicensePlate, GameHandler.ini_displaytime);
                        _mlog_loop3 = 6;
                        switch (endingvariant)
                        {
                            case Globals.CalloutEnd.NORMAL:
                    ...
                    case 8:
                        ...
                        FiberHandler.BackToAnimation(_ped_civ, Content.HumanList.First(t => t._calloutid == _area._calloutid && t._id == 3)._heading, "friends@frt@ig_1", "trevor_impatient_wait_4", AnimationFlags.Loop);
                        switch (endingvariant)
                        {
                            case Globals.CalloutEnd.NORMAL:
