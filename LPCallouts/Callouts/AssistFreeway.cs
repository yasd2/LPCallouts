            ...
            if (Rage.Game.IsKeyDown(GameHandler.ini_action))
            {
                ...
                switch (_mlog_loop4)
                {
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        GameHandler.PlayerChat(3, Dialog + _veh_suspect.Model.Name + " "+ AssistCyclistTranslation.TEXT[5] +" " + _veh_suspect.LicensePlate, GameHandler.ini_displaytime);
                        _mlog_loop4 = 6;
                        switch (endingvariant)
                        {
                            case Globals.CalloutEnd.NORMAL:
                                SuspectHandler.GetLocation(_veh_suspect.Model.Name, _veh_suspect.LicensePlate, _veh_suspect, _ped_suspect, _ped_civ3, _blip_suspect, _suspect_location._ped_position, SuspectData.FullName);
                                break;
                            case Globals.CalloutEnd.ABANDON:
                            case Globals.CalloutEnd.TRAFFICSTOP:
                                SuspectHandler.GetLocation(_veh_suspect.Model.Name, _veh_suspect.LicensePlate, _veh_suspect, _ped_civ3);
                                break;
                        }
                        break;
                    case 6:
